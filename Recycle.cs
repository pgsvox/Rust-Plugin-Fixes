...
namespace Oxide.Plugins
{
    ...
    class Recycle : RustPlugin
    {
        ...
        bool SalvageItem (BasePlayer player, Item item)
        {
            if (item == null || item.info.shortname == null || item.info.shortname == "scrap"){
                return false;
            }
			...
			{
				Puts("Can't recycle " + item.info.shortname);
				ShowNotification (player, "You can't recycle " + item.info.shortname);
				return false;
			}
