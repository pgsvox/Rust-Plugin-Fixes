...
namespace Oxide.Plugins
{
    ...
    class HeliControl : RustPlugin
    {
        ...
        private BaseHelicopter callHeli(Vector3 coordinates = new Vector3(), Vector3 startcoordinates = new Vector3(),bool forced = true)
        {
			Vector3 gotoloc = Vector3.zero;
            BaseHelicopter heli =  null;

			heli = (BaseHelicopter)GameManager.server.CreateEntity(heliPrefab, new Vector3(), new Quaternion(), true);
			...
			if (coordinates != Vector3.zero && coordinates == startcoordinates)
			{
				//float ran1 = UnityEngine.Random.Range(PatrolHelicopterAI.strafe_approach_range + 5.0F, PatrolHelicopterAI.strafe_approach_range + 50.0F);
				float ran1 = UnityEngine.Random.Range( 75.0F, 190.0F);
				float ran2 = UnityEngine.Random.Range(5.0f, 40.0f);
				float ran3 = UnityEngine.Random.Range(75.0F, 190.0F);
				//float ran3 = UnityEngine.Random.Range(PatrolHelicopterAI.strafe_approach_range + 5.0F, PatrolHelicopterAI.strafe_approach_range + 50.0F);
				float ran1sign = UnityEngine.Random.Range(0f,1f) >= 0.5f ? -1f : 1f;
				float ran3sign = UnityEngine.Random.Range(0f,1f) >= 0.5f ? -1f : 1f;
				gotoloc = startcoordinates + new Vector3(ran1 * ran1sign, ran2, ran3 * ran3sign); // add some randomization to make it less predictable
				heliAI.SetTargetDestination(coordinates + new Vector3(0f, 10f, 0f), 0.25f);
			}
			else if (coordinates != Vector3.zero) heliAI.SetInitialDestination(coordinates + new Vector3(0f, 10f, 0f), 0.25f);
			
            if (forced) forceCalled.Add(heliAI.helicopterBase);
			heli.Spawn();
			
			if (gotoloc != Vector3.zero)
			{
				heli.transform.position = (gotoloc);
				heli.Update();
				if (coordinates != Vector3.zero) 
				{
					heliAI.SetAimTarget(coordinates + new Vector3(0f, 0f, 0f),false);
					// heliAI.State_Strafe_Enter(coordinates + new Vector3(0f, 0f, 0f),false);
					heliAI.FireGun(coordinates, 0.25f, false);
					heliAI.AIThink();
				}
			}
			else if (startcoordinates != Vector3.zero)
			{
				heli.transform.position = (startcoordinates);
				heli.Update();				
			}
        ...
        {
            var heli = (CH47HelicopterAIController)GameManager.server.CreateEntity(chinookPrefab, new Vector3(0f, 100f, 0f), new Quaternion(), true);
            if (heli == null) return null;
            ...
            heli.Spawn();
            if (coordinates != Vector3.zero) timer.Once(1f, () => { if (heli != null && !heli.IsDestroyed) SetDestination(heli, coordinates + new Vector3(0f, 10f, 0f));});
            return heli;
        }

        private List<BaseHelicopter> callHelis(int amount, Vector3 coordinates = new Vector3(), Vector3 startcoordinates = new Vector3(), bool forced = true)
        {
            if (amount < 1) return null;
            var listHelis = new List<BaseHelicopter>(amount);
            for (int i = 0; i < amount; i++) listHelis.Add(callHeli(coordinates, startcoordinates, forced));
            return listHelis;
        }
        ...
        private void cmdCallToPlayer(BasePlayer player, string command, string[] args)
        {
            ...
            try
            {
                ...
                int num = 1;
				string checkonplayer = null;
				
                if (args.Length >= 2 && HasPerms(player.UserIDString, "callheli") && !int.TryParse(args[1], out num)) num = 1;
                if (args.Length == 3) checkonplayer = args[2].ToLower().Substring(0,1);
				
				
				if (string.IsNullOrEmpty(checkonplayer) || checkonplayer == "f" || checkonplayer == "0" || checkonplayer == "n")
					newHelis = callHelis(num, target.transform.position);
				else
					newHelis = callHelis(num, target.transform.position, target.transform.position);
        ...
        {
			var arglength = arg?.Args?.Length ?? 0;
            ...
            if (arg.Args[0].ToLower() == "pos" && arglength < 4)
            {
            ...
            if (arg.Args[0].ToLower() == "pos")
            {
                ...
                if (arglength > 4) if (!int.TryParse(arg.Args[4], out callNum)) callNum = 1;
            ...
            if (arglength >= 2 && !int.TryParse(arg.Args[1], out num)) num = 1;
            if (arglength == 3) checkonplayer = arg.Args[2].ToLower().Substring(0,1);
			
			//Puts("Called on target location " + checkonplayer + " for target" + target.displayName);
			
			if (string.IsNullOrEmpty(checkonplayer) || checkonplayer == "f" || checkonplayer == "0" || checkonplayer == "n")
				newHelis = callHelis(num, (target?.transform?.position ?? Vector3.zero));
			else
				newHelis = callHelis(num, (target?.transform?.position ?? Vector3.zero), (target?.transform?.position ?? Vector3.zero));
