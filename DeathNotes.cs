...
namespace Oxide.Plugins
{
    ...
    class DeathNotes : RustPlugin
    {
        ...
        List<string> selfInflictedDeaths = new List<string> { "Cold", "Drowned", "Heat", "Suicide", "Generic", "Poison", "Radiation", "Thirst", "Hunger", "Fall" };
        ...
        List<Regex> regexTags = new List<Regex>
        {
            ...
            new Regex(@"\[[1-9]{5,8}\/[1-9]{5,8}\]", RegexOptions.Compiled)			
        };
        ...
        bool UseSimpleUI;
        bool SnowNpcBattles;
		bool ShowNPCName; // Show NPC Names
        ...
        class Attacker
        {
			...
			public bool ShowNPCName = dn.ShowNPCName;

            public string TryGetName()
            {
                if (entity == null || type == AttackerType.Invalid || type == null)
                    return "No Attacker";
                else if (type == AttackerType.Scientist)
				{
					if (ShowNPCName == true)
						try
						{
							return "NPC " + entity.ToPlayer().displayName;
						}
						catch
						{
							return "Scientist";
                ...
                else if (type == AttackerType.Zombie)
				{
					if (ShowNPCName == true)
						try
						{
							return "Zombie " + entity.ToPlayer().displayName;
						}
						catch
						{
							return "Zombie";
                ...
                else if (type == AttackerType.Player)
                    return entity.ToPlayer().displayName;
                else if (type == AttackerType.BradleyAPC)
                    return "BradleyAPC";
                ...
                else if (type == AttackerType.Animal)
                {
                    if (entity.name.Contains("boar"))
                        return "Boar";
                    else if (entity.name.Contains("horse"))
                        return "Horse";
                        ...
                        return "Bear";
                    else if (entity.name.Contains("zombie"))
                        return "Zombie";
					else
						return "No Attacker";
                }
                else if (type == AttackerType.Structure)
                {
                    ...
                    else if (entity.name.Contains("barricade.woodwire.prefab"))
                        return "Barbed Wooden Barricade";
                    else if (entity.name.Contains("barricade.metal.prefab"))
                        return "Metal Barricade";
                    else if (entity.name.Contains("wall.external.high.wood.prefab"))
                        return "High External Wooden Wall";
                    else if (entity.name.Contains("wall.external.high.stone.prefab"))
                        return "High External Stone Wall";
                    else if (entity.name.Contains("gates.external.high.wood.prefab"))
                ...
                else if (type == AttackerType.Trap)
                {
                    if (entity.name.Contains("beartrap.prefab"))
                        return "Snap Trap";
                    else if (entity.name.Contains("landmine.prefab"))
                        return "Land Mine";
                    else if (entity.name.Contains("spikes.floor.prefab"))
            ...
            public AttackerType TryGetType()
            {
                ...
                else if (entity is BaseHelicopter)// entity.name.Contains("patrolhelicopter.prefab") && !entity.name.Contains("gibs"))
                    return AttackerType.Helicopter;
                else if (entity is NPCMurderer || entity.ShortPrefabName == "zombie")
                ...
                else if (entity.name.Contains("barricades/") || entity.name.Contains("external.high"))
                    return AttackerType.Structure;
                else if (entity.name.Contains("beartrap.prefab") || entity.name.Contains("landmine.prefab") || entity.name.Contains("spikes.floor.prefab"))
                    return AttackerType.Trap;
                else if (entity.name.Contains("autoturret_deployed.prefab"))
        ...
        class Victim
        {
            ...
            public string TryGetName()
            {
                if (type == VictimType.Invalid || type == null)
                    return "No Victim";
				else if (type == VictimType.Scientist)
				{
					if (ShowNPCName == true)
						try
						{
                ...
                else if (type == VictimType.Zombie)
				{
					if (ShowNPCName == true)
						try
						{
                ...
                else if (type == VictimType.Player)
                    return entity.ToPlayer().displayName;
                else if (type == VictimType.BradleyAPC)
                    return "BradleyAPC";
            ...
            public VictimType TryGetType()
            {
                if (entity == null)
					...
					return VictimType.Scientist;
                else if ((bool)entity?.name?.Contains("agents/"))
        ...
        class DeathData
        {
            ...
            public DeathReason TryGetReason()
            {
                ...
                if (victim.type == VictimType.BradleyAPC)
                ...
                else if (attacker.type == AttackerType.Scientist)
                ...
                else if (weapon == "F1 Grenade" || weapon == "Survey Charge" || weapon == "Beancan Grenade" || weapon == "Timed Explosive")
        ...
        void LoadConfig()
        {
            ...
            SetConfig("Settings", "Show NPC Names", true);
            ...
            SetConfig("Settings", "Message Color", "#BBFFBB");
            ...
            SetConfig("Messages", "Cold", new List<object> { "{victim} became an ice block." });
            ...
            SetConfig("Messages", "Generic", new List<object> { "Death took {victim} with him." });
            ...
            SetConfig("Messages", "BradleyAPC", new List<object> { "{victim} was shot to pieces by a {attacker}." });
            SetConfig("Messages", "BradleyAPCDeath", new List<object> { "The {victim} was taken down." });
            SetConfig("Messages", "Animal", new List<object> { "A {attacker} followed {victim} until it finally caught them." });
            SetConfig("Messages", "AnimalDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." });
            SetConfig("Messages", "Scientist", new List<object> { "A {attacker} dissected {victim}." });
            ...
            SetConfig("Messages", "Generic Sleeping", new List<object> { "Death took sleeping {victim} with him." });
			...
			ShowNPCName =  GetConfig(true, "Settings", "Show NPC Names");
            ...
            MessageColor = GetConfig("#BBFFBB", "Settings", "Message Color");
            ...
            Messages = GetConfig(new Dictionary<string, object>
            {
                ...
                { "BradleyAPC", new List<object> { "{victim} was shot to pieces by a {attacker}." }},
                { "BradleyAPCDeath", new List<object> { "The {victim} was taken down." }},
                { "Animal", new List<object> { "A {attacker} followed {victim} until it finally caught them." }},
                { "AnimalDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." }},
                { "Scientist", new List<object> { "A {attacker} hunted down {victim}." }},
        ...
        void GetInfo(BasePlayer player)
        {
            webrequest.EnqueueGet("http://oxidemod.org/plugins/819/", (code, response) => {
                ...
                SendChatMessage(player, $"<size=25><color=#C4FF00>DeathNotes</color></size> <size=20><color=#BBFFBB>by LaserHydra</color>{Environment.NewLine}<color=#BBFFBB>Latest <color=#C4FF00>{version_published}</color>{Environment.NewLine}Installed <color=#C4FF00>{version_installed}</color></color></size>");
            }, this);
        }
        ...
        void OnEntityTakeDamage(BaseCombatEntity victim, HitInfo info)
        {
            if (victim?.ToPlayer() != null && info?.Initiator?.ToPlayer() != null &&
			    !(victim is BaseNpc) && !(victim is NPCPlayerApex) && !(victim is NPCPlayer))
            {
                NextTick(() =>
                {
        ...
        void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
            ...
            if (!(victim is BaseCorpse) && (victim?.name?.Contains("helicopter") ?? false) && (victim?.name?.Contains("bradley") ?? false))
            {
				//Puts(victim?.name);
				//if (!(victim is BaseCorpse)) Puts("Not a Corpse");
				//if (victim?.name?.Contains("helicopter") ?? false) Puts("Not helicopter");
				//if (victim?.name?.Contains("bradley") ?? false) Puts("Not bradley");
				return;
			}

            var npc1 = info?.Initiator as BaseNpc;
            var npc2 = victim as BaseNpc;
			var scientistnpcapex = victim as NPCPlayerApex;
			var scientistnpc = victim as NPCPlayer;

            if (!SnowNpcBattles && npc1 != null && npc2 != null)
                return;

            if (victim.ToPlayer() != null  && scientistnpcapex == null && scientistnpc == null && npc2 == null )
            {
                if (victim.ToPlayer().IsWounded())
			...
			{
				//Puts("victim?.name: " + victim?.name);
				//Puts("IITP: " + info?.Initiator?.ToPlayer());
                return;
			}
            DeathData data = new DeathData();
			...
			//if (debug)
			//{
			//	Puts("data.victim.entity.name: " + data.victim.entity.name);
			//	Puts("data.victim.entity.ShortPrefabName: " + data.victim.entity.ShortPrefabName);
			//}

            data.victim.type = data.victim.TryGetType();

            if (data.victim.type == VictimType.Invalid || data.victim.type == null)
			{
				//Puts ("data.victim.type: " + data.victim.type);
                return;
			}

            data.victim.name = StripTags(data.victim.TryGetName());
            if (data.victim.name == "No Victim"  || data.victim.name == null)
			{
				//Puts ("data.victim.name: " + data.victim.name);
                return;
			}
            ...
            if (data.weapon.Contains("Heli"))
            {
                data.attacker.name = "Patrol Helicopter";
				data.attacker.type = AttackerType.Helicopter;
                data.reason = DeathReason.Helicopter;
            }

			if (data.weapon.Contains("MainCannonShell"))
            {
				data.attacker.name = "Bradley";
				data.attacker.type = AttackerType.BradleyAPC;
				data.reason = DeathReason.BradleyAPC;
            }

			//Puts(data.attacker.TryGetName());
			//Puts(data.attacker.name);
			
            if ((data.attacker.name == "No Attacker" || data.attacker.name == null) && 
			    data.victim.type != VictimType.Helicopter && data.victim.type != VictimType.BradleyAPC)
			{
				//Puts("1 No Attacker , Victim name , weapon: " + data.victim.name + " , " + data.weapon);
                return;
			}
			
			if (data.weapon == "No Weapon" && (data.attacker.name == "No Attacker" || data.attacker.type == AttackerType.Player) &&
				data.victim.type != VictimType.Helicopter && data.victim.type != VictimType.BradleyAPC)
			{
				//Puts("2 No Attacker and weapon , Victim name: " + data.victim.name);
                return;
			}
